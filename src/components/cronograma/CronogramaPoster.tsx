...
interface CronogramaPosterProps {
  ...
  tagline?: string;
  highlight?: string;
  /** Scale aplicado a la versión preview. 1 = tamaño real 1080x1920 (para export). */
  scale?: number;
  /** Si true, el contenedor auto-escala con CSS según ancho de pantalla. */
  responsive?: boolean;
}
...
export const CronogramaPoster = forwardRef<HTMLDivElement, CronogramaPosterProps>(
  function CronogramaPoster(
    ...
    const computedTransform = scale
      ? `scale(${scale})`
      : responsive
        ? 'scale(min(0.5, calc((100vw - 64px) / 1080)))'
        : undefined;

    const negativeMargin = scale
      ? `${-H * (1 - scale)}px`
      : responsive
        ? `calc(-1920px * (1 - min(0.5, (100vw - 64px) / 1080)))`
        : undefined;
